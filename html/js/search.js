const indexName = "schnitzler-interviews";

const typesenseInstantsearchAdapter = new TypesenseInstantSearchAdapter({
  server: {
    apiKey: "dwy0fPmmQwm0yRLr6ZGqMGIPOV1wYKMm",
    nodes: [
      {
...
search.addWidgets([
    ...
    templates: {
      ...
      item(hit, { html, components }) {
        return html` <div>
          ...
          <br />
          ${hit.orgs.map(
            (item) => html`<a href="${item.id}.html" class="pe-2"><i class="bi bi-building-gear pe-1"></i>${item.label}</a>`
          )}
          <br />
        </div>`;
      },
  ...
  instantsearch.widgets.panel({
    ...
    showMoreLimit: 50,
    limit: 10,
    searchablePlaceholder: "Suche nach Institutionen",
    cssClasses: DEFAULT_CSS_CLASSES,
  }),
